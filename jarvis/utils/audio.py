...
@lru_cache(maxsize=1)
def ffmpeg_path() -> str:
    """Путь к bundled ffmpeg бинарю (скачивается imageio-ffmpeg при первом запуске)."""
    try:
...
async def play_audio_bytes(audio_bytes: bytes, input_format: str = "mp3") -> None:
    """Декодирует аудио через ffmpeg в PCM и проигрывает через sounddevice.

    Никаких временных файлов, никаких внешних плееров.
    """
    if not audio_bytes:
        return
